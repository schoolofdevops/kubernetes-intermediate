...
must_haves:
  truths:
    ...
    - "Module 2 builds on Module 1 state (uses existing cluster with scheduling rules)"
  artifacts:
  ...
  key_links:
    - from: "docs/section-2/03-lab.md"
      ...
      via: "References Voting App from Module 0/1 as base"
      pattern: "vote"
    - from: "docs/section-2/03-lab.md"
...
Follow templates/lab-template.md 8-section structure. Duration: 50-60 minutes. Builds on Module 1 state.
...
- **Cleanup**: This is the last carry-forward module. Clean up KEDA and HPA resources but keep the base cluster:
  "Module 3 (Gateway API) starts with a fresh deployment approach. Clean up autoscaling resources:
  `kubectl delete hpa vote-hpa`, `kubectl delete scaledobject worker-scaledobject`, delete load generator pod.
  Keep KIND cluster and base Voting App running."
- **Troubleshooting**:
...
- Lab builds on Module 1 state (references existing scheduling)
